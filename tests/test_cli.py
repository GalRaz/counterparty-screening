...
def test_media_add_malformed_stdin_exits_1(capsys, monkeypatch, wired):
    setup_case(capsys, monkeypatch)
    code, out = run("media", "add", "E1", "alex-example", stdin="not json", monkeypatch=monkeypatch, capsys=capsys)
    assert code == 1 and "JSON" in out.err
    code, out = run("media", "add", "E1", "alex-example", stdin='["x"]', monkeypatch=monkeypatch, capsys=capsys)
    assert code == 1
...
def test_run_c_after_missing_key_replaces_the_not_run_entry(capsys, monkeypatch, wired):
    setup_case(capsys, monkeypatch)
    monkeypatch.setattr(cli.config, "get_secret", lambda name: None)
    assert run("run", "C", "E1", "--subject", "alex-example", capsys=capsys)[0] == 4
    monkeypatch.setattr(cli.config, "get_secret", lambda name: "NSKEY")
    assert run("run", "C", "E1", "--subject", "alex-example", capsys=capsys)[0] == 0
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
    ...
    text = (wired / "E1" / "summary.md").read_text()
    row = [l for l in text.splitlines() if l.startswith("| Alex Example |")][0]
...
def test_media_rm_removes_by_index(capsys, monkeypatch, wired):
    ...
    assert run("media", "add", "E1", "alex-example", stdin=json.dumps([item, other]),
               monkeypatch=monkeypatch, capsys=capsys)[0] == 0
    code, out = run("media", "rm", "E1", "alex-example", "0", capsys=capsys)
    assert code == 0 and "Councillor fined" in out.out
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
    assert [m["title"] for m in rec["media_items"]] == ["Unrelated arrest"]
    code, out = run("media", "rm", "E1", "alex-example", "5", capsys=capsys)
    assert code == 1 and "out of range" in out.err


def test_gap_add_records_a_coverage_gap(capsys, monkeypatch, wired):
    setup_case(capsys, monkeypatch)
    code, out = run("gap", "add", "E1", "alex-example", "no Dzongkha-language sources reachable", capsys=capsys)
    assert code == 0
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
...
def test_gap_rm_removes_by_index_and_rejects_out_of_range(capsys, monkeypatch, wired):
    setup_case(capsys, monkeypatch)
    code, out = run("gap", "add", "E1", "alex-example", "first gap", capsys=capsys)
    assert code == 0
    code, out = run("gap", "add", "E1", "alex-example", "second gap", capsys=capsys)
    assert code == 0
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
    assert rec["coverage_gaps"] == ["first gap", "second gap"]
    code, out = run("gap", "rm", "E1", "alex-example", "0", capsys=capsys)
    assert code == 0 and "first gap" in out.out
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
    assert rec["coverage_gaps"] == ["second gap"]
    code, out = run("gap", "rm", "E1", "alex-example", "5", capsys=capsys)
    assert code == 1 and "out of range" in out.err
...
def test_layerb_close_refuses_to_downgrade_full_to_reduced(capsys, monkeypatch, wired):
    ...
    qf = wired / "q.txt"
    qf.write_text('"Alex Example"\n')
    code, out = run("layerb", "close", "E1", "alex-example", "--mode", "reduced",
...
def test_layerb_close_allows_declaring_full_when_computed_is_reduced(capsys, monkeypatch, wired):
    ...
    qf = wired / "q.txt"
    qf.write_text('"Alex Example"\n')
    code, out = run("layerb", "close", "E1", "alex-example", "--mode", "full",
                    "--queries-file", str(qf), "--languages", "en", capsys=capsys)
    assert code == 0
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
...
def test_layerb_close_failed_requires_reason_and_records_gap(capsys, monkeypatch, wired):
    ...
    qf = wired / "q.txt"
    qf.write_text('"Alex Example"\n')
    code, out = run("layerb", "close", "E1", "alex-example", "--mode", "full", "--status", "failed",
                    "--queries-file", str(qf), "--languages", "en", capsys=capsys)
...
def test_candidate_set_dispositions_watchlist_candidate(capsys, monkeypatch, wired):
    setup_case(capsys, monkeypatch)
    assert run("run", "A", "E1", "--subject", "alex-example", capsys=capsys)[0] == 0
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
    cand_id = rec["watchlist_candidates"][0]["id"]
    code, out = run("candidate", "set", "E1", "alex-example", cand_id, "--assessment", "false_positive",
...
def test_candidate_set_exits_1_when_id_not_found(capsys, monkeypatch, wired):
    setup_case(capsys, monkeypatch)
    assert run("run", "A", "E1", "--subject", "alex-example", capsys=capsys)[0] == 0
    code, out = run("candidate", "set", "E1", "alex-example", "NOPE", "--assessment", "true_match",
...
def test_candidate_set_exits_1_when_by_is_empty(capsys, monkeypatch, wired):
    ...
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
    cand_id = rec["watchlist_candidates"][0]["id"]
    code, out = run("candidate", "set", "E1", "alex-example", cand_id, "--assessment", "true_match",
                    "--by", "  ", capsys=capsys)
...
def test_candidate_set_layer_c_by_index(capsys, monkeypatch, wired):
    setup_case(capsys, monkeypatch)
    assert run("run", "C", "E1", "--subject", "alex-example", capsys=capsys)[0] == 0
    code, out = run("candidate", "set", "E1", "alex-example", "0", "--layer", "C",
    ...
    code, out = run("candidate", "set", "E1", "alex-example", "5", "--layer", "C",
                    "--assessment", "true_match", "--by", "Jane Analyst", capsys=capsys)
    assert code == 1 and "out of range" in out.err
...
def test_subject_alias_appends_and_dedupes(capsys, monkeypatch, wired):
    setup_case(capsys, monkeypatch)
    code, out = run("subject", "alias", "E1", "alex-example", "Alexander Example", capsys=capsys)
    assert code == 0
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
    assert rec["subject"]["aliases"] == ["A. Example", "Alexander Example"]
    code, out = run("subject", "alias", "E1", "alex-example", "Alexander Example", capsys=capsys)
    assert code == 1 and "Alexander Example" in out.err
    code, out = run("subject", "alias", "E1", "alex-example", "A. Example", capsys=capsys)
    assert code == 1
...
def test_run_c_scans_each_alias_and_drops_the_alias_gap(capsys, monkeypatch, wired):
    assert run("case", "new", "E1", "--commissioning-party", "Example Authority", capsys=capsys)[0] == 0
    assert run("subject", "add", "E1", "--type", "person", "--name", "Alex Example", "--jurisdiction", "AU",
               "--id", "dob=1970@passport copy", "--alias", "A. Example", "--alias", "Alexander Example",
               capsys=capsys)[0] == 0
    code, out = run("run", "C", "E1", "--subject", "alex-example", capsys=capsys)
    assert code == 0
    rec = json.loads(run("record", "show", "E1", "alex-example", capsys=capsys)[1].out)
    aliases = rec["layer_c"]["alias_scans"]
    assert [a["alias"] for a in aliases] == ["A. Example", "Alexander Example"]
